...
int main(void)
{
	...
	int32_t status = AUTHENTICATION_SUCCESSFUL;
	...
	//Decrypt with Key1
	status = AES_CCM_Decrypt((uint8_t *)HeaderMessage, HeaderLength, InputMessage, InputLength, Key1, IV, sizeof(IV), OutputMessage, &OutputMessageLength, (int32_t  *)AuthenticationTAGLength);
     ...
     if (status == AUTHENTICATION_SUCCESSFUL)
     {
        ...
        //Encrypt with KEY2
 		uint8_t EncryptedMessage[MAX_OUTPUT_SIZE];
 		uint32_t EncryptedMessageLength = 0;


 		status = AES_CCM_Encrypt((uint8_t  *)HeaderMessage, HeaderLength,OutputMessage,
 								  OutputMessageLength, Key2, IV, sizeof(IV), EncryptedMessage,
								   (int32_t *)EncryptedMessageLength, &AuthenticationTAGLength);

 		if (status == AES_SUCCESS)
 		{
 			HAL_UART_Transmit(&huart3, (uint8_t *)"\nEncrypted data: ", strlen("\nEncrypted data: "), 1000);
 			HAL_UART_Transmit(&huart3, EncryptedMessage, EncryptedMessageLength, 1000);
