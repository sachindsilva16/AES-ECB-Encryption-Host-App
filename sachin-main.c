...
const uint8_t HeaderMessage[]={0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07};

uint32_t HeaderLength = sizeof(HeaderMessage);



uint8_t Key1[CRL_AES128_KEY]={0x74,0x65,0x73,0x74,0x74,0x65,0x73,0x74,0x74,0x65,0x73,0x74,0x74,0x65,0x73,0x74};
uint8_t Key2[CRL_AES128_KEY]={0x74,0X65,0X73,0X74,0X67,0X6F,0X6F,0X64,0X74,0X65,0X73,0X74,0X67,0X6F,0X6F,0X64};


uint8_t IV[] = {0x10,0x11,0x12,0x13,0x14,0x15,0x1,0x17};
...
int32_t AuthenticationTAGLength = 4;
...
int main(void)
{
  ...
  uint8_t InputMessage[PLAINTEXT_LENGTH];
  uint8_t InputLength = 0;
HAL_UART_Receive(&huart3, InputMessage, sizeof(InputMessage), 30000);
	InputLength = sizeof(InputMessage);


  // DECRYPT THE ENCRYPTED DATA FROM THE HOST APP USING KEY 1

  status = AES_CCM_Encrypt((uint8_t *)HeaderMessage,HeaderLength,InputMessage,InputLength,Key1,IV,sizeof(IV),OutputMessage,&OutputMessageLength,(int32_t *)AuthenticationTAGLength);

  if(status == AES_SUCCESS){
uint8_t OutputAscii[OutputMessageLength * 2];
BinaryToHex(OutputMessage, OutputMessageLength, OutputAscii);



UART_Transmit_ASCII(OutputAscii, OutputMessageLength*2, &huart3);
  } else {
    HAL_UART_Transmit(&huart3,(uint8_t *)"Encryption failed!",strlen("Encryption failed!"),1000);
  }
